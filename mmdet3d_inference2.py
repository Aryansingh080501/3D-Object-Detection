...
def main(args):
    ...
    elif args.dataset == 'custom_pcd':
        # Custom PCD dataset: convert .pcd -> KITTI-style .bin and reuse lidar pipeline
        print(f"Using custom PCD dataset mode with base folder: {args.input_path}")
        if not os.path.isdir(args.input_path):
            print(f"Error: custom_pcd base folder does not exist: {args.input_path}")
            return

        # Where to store temporary .bin files
        tmp_bin_dir = os.path.join(args.out_dir, "custom_pcd_bin")
        os.makedirs(tmp_bin_dir, exist_ok=True)

        # List .pcd files
        pcd_files = list_custom_pcd_files(args.input_path)
        if args.max_samples is not None:
            pcd_files = pcd_files[:args.max_samples]

        print(f"Found {len(pcd_files)} custom PCD samples under {args.input_path}")

        for idx, pcd_path in enumerate(pcd_files):
            frame_id = Path(pcd_path).stem
            print(f"  [{idx+1}/{len(pcd_files)}] Preparing custom PCD sample: {frame_id}")

            # Load .pcd as normalized [N,4] (x,y,z,intensity)
            lidar_points = load_pcd_as_lidar_points(pcd_path)

            # Write to KITTI-style .bin (float32)
            bin_path = os.path.join(tmp_bin_dir, f"{frame_id}.bin")
            lidar_points.astype(np.float32).tofile(bin_path)

            # Build input dict using the .bin path (same as any lidar file)
            input_dict = build_input_dict(
                bin_path, args.modality, args.img_dir, args.calib_dir, args.gt_label_dir
            )
            inputs_list.append(input_dict)

        print(f"Prepared {len(inputs_list)} custom_pcd inputs (converted to .bin).")
    
    else:
        print("Using manual path mode (any dataset).")
        if os.path.isfile(args.input_path):
            primary_file = args.input_path
            inputs_list.append(
...
if __name__ == "__main__":
    ...
    "--dataset",
    type=str,
    choices=["kitti", "nuscenes", "custom_pcd"],
    help="Dataset type: kitti, nuscenes, or custom_pcd",
    )
    ...
    type=int,
    default=None,
    help="Maximum number of samples to process (for custom_pcd / batch runs).",
    )
    ...
    def list_custom_pcd_files(input_dir):
        """Return a sorted list of .pcd files under input_dir."""
        pcd_paths = sorted(glob.glob(os.path.join(input_dir, "*.pcd")))
        if not pcd_paths:
            raise FileNotFoundError(f"No .pcd files found under {input_dir}")
        return pcd_paths


    def load_pcd_as_lidar_points(pcd_path):
        """
        Load a .pcd file with Open3D and convert to [N,4] (x,y,z,intensity),
        roughly normalized to a KITTI-like LiDAR range.

        - Center the cloud at the origin
        - Scale to keep x,y within about [-50, 50]
        - Clamp z into [-3, 1]
        - Randomly downsample if too many points (to avoid voxelization issues)
        """
        MAX_POINTS = 50_000

        pcd = o3d.io.read_point_cloud(pcd_path)
        pts = np.asarray(pcd.points, dtype=np.float32)  # [N,3] typically

        if pts.ndim != 2 or pts.shape[1] < 3 or pts.shape[0] == 0:
            raise ValueError(f"Unexpected point shape {pts.shape} in {pcd_path}")

        # Use only x, y, z
        pts = pts[:, :3]

        # Random downsampling if extremely dense
        ...
        # 1) Center cloud around origin
        mean = pts.mean(axis=0, keepdims=True)
        pts_centered = pts - mean

        # 2) Scale to keep x,y roughly within [-50, 50]
        max_xy = np.max(np.abs(pts_centered[:, :2]))
        if max_xy > 0:
            scale = min(50.0 / max_xy, 1.0)  # shrink if too large
            pts_centered *= scale

        # 3) Clamp z into a LiDAR-like vertical range
        pts_centered[:, 2] = np.clip(pts_centered[:, 2], -3.0, 1.0)

        # 4) Add dummy intensity
        intens = np.zeros((pts_centered.shape[0], 1), dtype=np.float32)
        lidar_points = np.concatenate([pts_centered, intens], axis=1)  # [N,4]
